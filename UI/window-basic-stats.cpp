...
void OBSBasicStats::Update()
{
	...
	auto additionalLiveStreamOutputs = main->GetAdditionalLiveStreamOutputs();

	if (additionalLiveStreamOutputs.empty() && outputLabels.size() > 2) {
		std::for_each(outputLabels.begin() + 2, outputLabels.end(),
			      [&](auto &labels) { RemoveOutputLabels(labels); });
		outputLabels.erase(outputLabels.begin() + 2, outputLabels.end());
	}
...
#define TBYTE (1024ULL * 1024ULL * 1024ULL * 1024ULL)
	...
	for (auto &liveOutput : additionalLiveStreamOutputs) {
		...
		auto it = std::find_if(outputLabels.begin(), outputLabels.end(),
				       [&](auto &it) { return it.name->text() == name; });
...
void OBSBasicStats::OutputLabels::Reset(obs_output_t *output)
{
	...
	weakOutput = OBSWeakOutputAutoRelease{obs_output_get_weak_output(output)};
