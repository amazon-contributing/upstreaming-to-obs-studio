...
BerryessaEveryMinute::BerryessaEveryMinute(QObject * /*parent*/, BerryessaSubmitter *berryessa,
	  ...
	  shared_counters_(std::make_shared<UsageInfoCounters>())
{
	...
	for (const auto &encoder : encoders) {
		...
		encoder_counters.push_back(OBSEncoderFrameCounters{
			OBSWeakEncoderAutoRelease(obs_encoder_get_weak_encoder(encoder)),
...
static void AddOBSStats(os_cpu_usage_info *info, OBSFrameCounters &frame_counters,
			std::vector<OBSEncoderFrameCounters> &encoder_counters, obs_data_t *event)
{
	obs_data_set_double(event, "obs_cpu_usage", os_cpu_usage_info_query(info));
	obs_data_set_double(event, "current_obs_fps", obs_get_active_fps());
	obs_data_set_int(event, "obs_memory_usage", os_get_proc_resident_size());
	...
	obs_data_set_int(event, "skipped_frames", skipped_frames - frame_counters.skipped);
	obs_data_set_int(event, "output_frames", output_frames - frame_counters.output);

	obs_data_set_int(event, "rendered_frames", rendered_frames - frame_counters.rendered);
	obs_data_set_int(event, "lagged_frames", lagged_frames - frame_counters.lagged);
	...
	for (size_t i = 0; i < encoder_counters.size(); i++) {
		...
		OBSEncoderAutoRelease encoder = obs_weak_encoder_get_encoder(counter.weak_output);
		if (!encoder)
			continue;
		auto video = obs_encoder_video(encoder);
		auto output_frames = video_output_get_total_frames(video);
		...
		obs_data_set_int(event, buffer->array, output_frames - counter.output);
		dstr_printf(buffer, "encoder%zu_skipped_frames", i);
		obs_data_set_int(event, buffer->array, skipped_frames - counter.skipped);
...
void BerryessaEveryMinute::fire()
{
	...
	QThreadPool::globalInstance()->start([shared_counters = shared_counters_,
					      ...
					      event = OBSData{event}] {
		AddOBSStats(shared_counters->obs_cpu_usage_info_.get(), shared_counters->frame_counters_,
...
#endif
		auto buffer = QDateTime::currentDateTimeUtc().toString(Qt::ISODateWithMs).toUtf8();
		obs_data_set_string(event, "time_utc", buffer.constData());

		QMetaObject::invokeMethod(berryessa,
					  [event, berryessa] { berryessa->submit("ivs_obs_stream_minute", event); });
	});
}
