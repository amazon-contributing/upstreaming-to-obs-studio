...
static void add_always_bool(BerryessaSubmitter *berryessa, const char *name, bool data)
{
	if (!berryessa)
		return;
...
static void add_always_string(BerryessaSubmitter *berryessa, const char *name, const char *data)
{
	if (!berryessa)
		return;
...
static OBSOutputs SetupOBSOutput(QWidget *parent, const QString &multitrack_video_name,
				 ...
				 std::vector<OBSEncoderAutoRelease> &video_encoders, const char *audio_encoder_id,
...
std::optional<std::vector<HashMismatch>> check_plugin_hash_mismatches(const char *text)
{
	using json = nlohmann::json;
	...
	try {
		...
		auto callback = [&](obs_module_t *module) {
			...
			for (auto &entry : hashes)
				try {
					auto module_hash = entry.template get<ModuleHash>();

					if (qstrnicmp(module_hash.Algorithm.c_str(), "SHA256", 7) != 0)
						continue;

					if (module_hash.Path.size() < file_name_length)
						continue;

					auto potential_file_name_offset = module_hash.Path.size() - file_name_length;

					auto potential_file_name_start =
						module_hash.Path.c_str() + potential_file_name_offset;
					if (qstrnicmp(potential_file_name_start, file_name, file_name_length) != 0)
						continue;

					if (qstrnicmp(module_hash.Hash.c_str(), hash, module_hash.Hash.size()) == 0)
						return;

					mismatches.push_back({file_name, hash, module_hash.Hash});
					return;
		...
		obs_enum_modules([](void *context,
				    obs_module_t *module) { (*static_cast<callback_t *>(context))(module); },
				 &callback);

		if (exception)
			throw *exception;
	} catch (const json::exception &exception) {
		blog(LOG_ERROR, "check_plugin_hash_mismatches: Error while processing plugin integrity json (%d): %s",
					     ...
					     std::optional<size_t> vod_track_mixer, std::optional<bool> use_rtmps)
{
	if (!berryessa_) {
		QMetaObject::invokeMethod(
			parent,
			[&] {
				berryessa_ = std::make_unique<BerryessaSubmitter>(parent,
	...
	if (berryessa_every_minute_ && berryessa_every_minute_->has_value()) {
		...
		QMetaObject::invokeMethod(parent, [bem = std::move(berryessa_every_minute_)] {});
	}
	...
	if (!custom_config_only) {
		...
		if (berryessa_) {
			add_always_string(berryessa_.get(), "config_id", go_live_config->meta.config_id.c_str());
		}
	}
	...
	auto outputs = SetupOBSOutput(parent, multitrack_video_name, dump_stream_to_file_config, output_config,
				      audio_encoders, video_encoder_group, video_encoders, audio_encoder_id,
				      main_audio_mixer, vod_track_mixer, extra_views_);
	...
	if (dump_stream_to_file_config && recording_output) {
		...
		for (auto &encoder : video_encoders) {
			recording_video_encoders.emplace_back(obs_encoder_get_ref(encoder));
		}
...
struct BerryessaEveryMinuteInitializationGuard {
	BerryessaEveryMinuteInitializationGuard() { future = guard.get_future().share(); };
...
void MultitrackVideoOutput::StartedStreaming(QWidget *parent)
{
	...
	if (berryessa_ && current) {
		...
		auto guard = std::make_shared<BerryessaEveryMinuteInitializationGuard>();
		berryessa_every_minute_initializer_.emplace(guard->GetFuture());

		QThreadPool::globalInstance()->start([=, bem = berryessa_every_minute_, berryessa = berryessa_.get(),
						      main_thread = QThread::currentThread(), guard = guard] {
			bem->emplace(parent, berryessa, video_encoders).moveToThread(main_thread);
		});
	}
}
...
static bool create_video_encoders(const GoLiveApi::Config &go_live_config,
				  std::shared_ptr<obs_encoder_group_t> &video_encoder_group,
				  std::vector<OBSEncoderAutoRelease> &video_encoders, obs_output_t *output,
				 ...
				 const std::map<std::string, video_t *> &extra_views)
{
	...
	if (!create_video_encoders(go_live_config, video_encoder_group, video_encoders, output, recording_output,
...
void StreamStopHandler(void *arg, calldata_t *data)
{
	...
	QMetaObject::invokeMethod(
		...
		[berryessa = QPointer{self->berryessa_.get()}, bem = std::move(self->berryessa_every_minute_),
		 object_releaser = std::move(object_releaser)] {
			if (berryessa) {
				berryessa->unsetAlways("config_id");
				berryessa->unsetAlways("stream_attempt_start_time");
			}
		},
		Qt::QueuedConnection);

	self->berryessa_every_minute_ = std::make_shared<std::optional<BerryessaEveryMinute>>(std::nullopt);
}
