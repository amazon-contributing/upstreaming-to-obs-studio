...
bool SimpleOutput::StartStreaming(obs_service_t *service)
{
	...
	if (obs_output_start(streamOutput)) {
		if (multitrackVideo && multitrackVideoActive)
			multitrackVideo->StartedStreaming(main);
		return true;
	}
...
void BasicOutputHandler::CreateSimulcastEncoders(const char *encoderId)
{
	...
	for (auto i = 0; i < 2; i++) {
		uint32_t width = video_output_get_width(obs_get_video()) / (1.5 + (.5 * i));
		width -= width % 2;

		uint32_t height = video_output_get_height(obs_get_video()) / (1.5 + (.5 * i));
		height -= height % 2;
...
{
	for (size_t i = 0; i < simulcastEncoders.size(); i++) {
		obs_data_set_int(videoSettings, "bitrate", videoBitrate / (2 * (i + 1)));
