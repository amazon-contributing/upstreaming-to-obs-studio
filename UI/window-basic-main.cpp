				       ...
				       multitrack_video_stop_cb stop_video, void *param)
{
	...
	multitrackVideoViews.push_back(MultitrackVideoViewInfo(name, start_video, stop_video, param));
}
...
{
	additionalStreamingOutputs.emplace_back(obs_output_get_weak_output(output));
...
{
	auto it =
		std::find(std::begin(additionalStreamingOutputs), std::end(additionalStreamingOutputs), removedOutput);
...
std::vector<OBSOutputAutoRelease> OBSBasic::GetAdditionalLiveStreamOutputs()
{
	...
	auto first_unalive = std::remove_if(std::begin(additionalStreamingOutputs),
	...
	additionalStreamingOutputs.erase(first_unalive, std::end(additionalStreamingOutputs));
