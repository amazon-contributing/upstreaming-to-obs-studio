...
extern std::optional<std::vector<HashMismatch>> check_plugin_hash_mismatches(const char *text);
...
static void check_plugin_hashes(OBSApp &program)
{
	BPtr<char> json = nullptr;
	{
		...
		if (!GetDataFilePath(filename->array, path)) {
			blog(LOG_WARNING, "check_plugin_hashes: Could not find plugin hashes file '%s'",
			     filename->array);
			return;
		}
	...
	blog(LOG_ERROR, "check_plugin_hashes: Mismatched plugin hashes detected:%s", mismatched_plugins->array);

	QMessageBox mb(QMessageBox::Critical, QTStr("PluginHashesMismatch.Title"),
		       QTStr("PluginHashesMismatch.Text").arg(mismatched_plugins->array),
		       QMessageBox::StandardButton::NoButton, program.GetMainWindow());
