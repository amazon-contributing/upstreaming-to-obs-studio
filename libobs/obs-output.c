...
static bool initialize_interleaved_packets(struct obs_output *output)
{
	...
	for (size_t i = 0; i < MAX_OUTPUT_VIDEO_ENCODERS; i++) {
		...
		for (size_t j = 0; j < MAX_OUTPUT_VIDEO_ENCODERS; j++) {
			struct video_dts_offsets_t *offset = &video_dts_offsets[j];
			if (offset->encoder_group && offset->encoder_group != encoder->encoder_group)
				continue;
	...
	for (size_t i = 0; i < MAX_OUTPUT_VIDEO_ENCODERS; i++) {
		...
		for (size_t j = 0; j < MAX_OUTPUT_VIDEO_ENCODERS; j++) {
			...
			if (offset->encoder_group != encoder->encoder_group)
				continue;
			output->video_dts_offsets[i] = video[i]->dts - video[i]->pts - offset->lowest_dts;
			break;
		}
	}
