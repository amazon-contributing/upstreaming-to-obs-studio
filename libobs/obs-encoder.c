...
bool obs_encoder_set_speaker_layout(obs_encoder_t *encoder, enum speaker_layout speakers)
{
	...
	encoder->speakers_override = audio_output_create_with_speaker_layout(encoder->media, encoder->speakers);
	return true;
}
	...
	if (audio) {
		...
		if (encoder->speakers != SPEAKERS_UNKNOWN) {
			encoder->speakers_override =
				audio_output_create_with_speaker_layout(encoder->media, encoder->speakers);
		}
	} else {
		encoder->media = NULL;
		encoder->timebase_num = 0;
...
audio_t *obs_encoder_audio(const obs_encoder_t *encoder)
{
	...
	return encoder->speakers_override ? encoder->speakers_override : encoder->media;
}
