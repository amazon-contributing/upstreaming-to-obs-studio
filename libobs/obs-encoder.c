...
static void
encoder_group_new_reconfigure_request(struct obs_encoder_group *group);
...
void obs_encoder_update(obs_encoder_t *encoder, obs_data_t *settings)
{
	...
	// Encoder groups need slightly more special handling
	// since encoders generally start new GOPs/insert IDR frames
	// when reconfigured, so we need to reconfigure them all
	// on the same frame.
	struct obs_encoder_group *group = encoder->encoder_group;
	pthread_mutex_lock(&group->mutex);
	bool first_reconfigure_request = group->reconfigure_request == 0;
	bool all_encoders_reconfigured = group->encoders_reconfigured ==
					 group->encoders.num;
	bool current_encoder_reconfigured =
...
{
	if (!obs_encoder_valid(encoder, "obs_encoder_set_audio_conversion"))
		return false;
	if (encoder->info.type != OBS_ENCODER_AUDIO) {
		blog(LOG_WARNING,
...
void handle_encoder_group_reconfigure_request(obs_encoder_t *encoder)
{
	...
	if (!new_reconfigure_request &&
	    group->encoders_updated_next_pts == group->num_encoders_started &&
		...
		if (group->encoders_reconfigured == group->encoders.num &&
		    group->reconfigure_again)
			encoder_group_new_reconfigure_request(group);

	} else if (new_reconfigure_request ||
		   encoder->cur_pts == group->next_pts) {
		encoder->last_reconfigure_request = group->reconfigure_request;

		int64_t overall_timestep =
			encoder->timebase_num * group->frame_rate_divisors_lcm;
		...
		if (next_pts % overall_timestep != 0)
			next_pts = (next_pts / overall_timestep + 1) *
				   overall_timestep;
